...
class DateFormatter {
   ...
   */
  formatDate() {
    if (this.isDateWithoutTime()) return this.formatWithTimezone();
    else return this.formatByTimezone();
  }
  ...
  formatByTimezone() {
    ...
    if (
      ...
      this.timezone == 'America/Bahia'
    ) {
      finalDate = this.date.tz(this.timezone).add(1, 'hours').format();
    } else {
      finalDate = this.date.tz(this.timezone).format();
    }
    return finalDate;
  }
